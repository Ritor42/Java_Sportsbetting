...
public class View implements IView {
	...
	public void printOutcomeOdds(List<SportEvent> events) {
		...
		for(SportEvent event : events) {
			for(Bet bet : event.getBets()) {
				for(Outcome outcome : bet.getOutcomes()) {
					for(OutcomeOdd odd : outcome.getOutcomeOdds()) {
						System.out.println(i + ": Sport event: " + event.getTitle() + " (start: " + event.getStartDate() + "), Bet: " + bet.getDescription() + ", Outcome: " + outcome.getDescription() + " Actual odd: " + odd.getValue() + ", Valid between " + odd.getValidFrom() + " and " + odd.getValidUntil());
						i++;
					}
				}
			}
	...
	public void printResults(Player player, List<Wager> wagers) {
		...
		for(Wager wager : wagers) {
			...
			System.out.println("Wager '" + outcome.getBet().getDescription() + "' of " + outcome.getDescription() + " [odd: "+odd.getValue()+", amount: " + wager.getAmount() + "], win: " + wager.isWin());
		}
	}
}
